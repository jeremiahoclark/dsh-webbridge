...
import { WebBridgeClient, WebBridgeClientError, } from "./client.js";
export { WebBridgeClient, WebBridgeClientError } from "./client.js";
 ...
 */
export function resolveConfig(config, environment = process.env) {
    ...
    if (sessionPrefix.trim() === '')
        throw new Error('tool-webbridge: sessionPrefix must not be empty');
    return {
 ...
 */
export function createWebBridgeTools(client, config) {
    const call = (action, args, exec) => client.command(action, args, sessionFor(exec, config.sessionPrefix), exec.signal);
    return [
        defineTool({
        ...
        defineTool({
            ...
            async execute(_args, exec) {
                try {
                    ...
                    if (error instanceof WebBridgeClientError && error.kind === 'cancelled')
                        throw error;
...
export function apply(ctx, config) {
    ...
    for (const tool of createWebBridgeTools(client, resolved))
        ctx.tools.register(tool);
}
function sessionFor(exec, prefix) {
    if (exec.agent === undefined)
        throw new Error('Kimi WebBridge commands require an owning dsh agent session');
