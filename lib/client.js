...
export class WebBridgeClient {
     ...
     */
    command(action, args, session, signal) {
        return this.#request(this.#commandURL, `command ${action}`, {
            method: 'POST',
            headers: {
     ...
     */
    status(signal) {
        return this.#request(this.#statusURL, 'status check', { headers: { accept: 'application/json' } }, signal);
    }
    async #request(url, operation, init, callerSignal) {
        ...
        try {
            ...
            if (!response.ok) {
                throw new WebBridgeClientError('http', `Kimi WebBridge ${operation} failed with HTTP ${response.status}: ${errorDetail(value)}`);
            }
            ...
            if (error instanceof WebBridgeClientError)
                throw error;
            if (callerSignal.aborted) {
                throw new WebBridgeClientError('cancelled', `Kimi WebBridge ${operation} was cancelled`, { cause: error });
            }
            if (timeout.signal.aborted) {
                throw new WebBridgeClientError('timeout', `Kimi WebBridge ${operation} timed out after ${this.#timeoutMs}ms`, { cause: error });
            }
            throw new WebBridgeClientError('network', `Cannot reach Kimi WebBridge at ${this.baseURL} for ${operation}: ${errorMessage(error)}. Ensure the daemon is running and the browser extension is connected.`, { cause: error });
        }
        finally {
            clearTimeout(timer);
        }
...
async function readBoundedText(response, maxBytes) {
    ...
    if (Number.isFinite(declaredLength) && declaredLength > maxBytes) {
        throw new WebBridgeClientError('protocol', `Kimi WebBridge response exceeds maxResponseBytes (${declaredLength} > ${maxBytes})`);
    }
    ...
    while (true) {
        ...
        if (bytes > maxBytes) {
            await reader.cancel();
            throw new WebBridgeClientError('protocol', `Kimi WebBridge response exceeds maxResponseBytes (${bytes} > ${maxBytes})`);
        }
...
function parseJson(text, operation) {
    ...
    try {
        ...
        throw new WebBridgeClientError('protocol', `Kimi WebBridge ${operation} returned invalid JSON: ${errorMessage(error)}`, { cause: error });
    }
}
function errorDetail(value) {
    return errorField(value) ?? objectString(value, 'message') ?? JSON.stringify(value);
}
function errorField(value) {
